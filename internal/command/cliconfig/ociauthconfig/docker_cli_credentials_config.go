...
func FindDockerCLIStyleCredentialsConfigs(ctx context.Context, env ConfigDiscoveryEnvironment) ([]CredentialsConfig, error) {
	...
	var err error
	prevPath := ""
	for filePath := range dockerCLIStyleAuthFileSearchLocations(env) {
...
func dockerCLIStyleAuthMatch(authsPropertyName string, wantRegistryDomain, wantRepositoryPath string) CredentialsSpecificity {
	...
	for i := range wantSegCount {
		var thisWant, thisGot string
		var moreGot bool
		thisWant, wantRemain, _ = strings.Cut(wantRemain, "/")
		thisGot, gotRemain, moreGot = strings.Cut(gotRemain, "/")
		if thisWant != thisGot {
			// Everything before i matched, but the remainder doesn't.
			return RepositoryCredentialsSpecificity(uint(i)) //nolint:gosec // iteration variable cannot possibly be negative
		}
		if !moreGot {
			break // no more segments to compare
		}
	}
	...
	return RepositoryCredentialsSpecificity(uint(gotSegCount)) //nolint:gosec // number of path segments in the input cannot possibly be negative
}

func dockerCLIStyleAuthFileSearchLocations(env ConfigDiscoveryEnvironment) iter.Seq[string] {
	...
	return func(yield func(string) bool) {
		// A primary location is only defined for Linux, MacOS, and Windows. Other
		// platforms rely exclusively on the other paths produced after this
		// if statement.
		if isLinux {
			if xdgRunDir := env.EnvironmentVariableVal("XDG_RUNTIME_DIR"); xdgRunDir != "" {
				keepGoing := yield(filepath.Join(xdgRunDir, "containers", "auth.json"))
				if !keepGoing {
					return
				}
			}
		} else if isWindows || isMacOS {
			keepGoing := yield(filepath.Join(homeDir, ".config", "containers", "auth.json"))
			if !keepGoing {
				return
			}
		}
		...
		keepGoing := yield(filepath.Join(xdgConfigHome, "containers", "auth.json")) // this might duplicate the primary location from above
		if !keepGoing {
			return
		}
		keepGoing = yield(filepath.Join(homeDir, ".docker", "config.json"))
		if !keepGoing {
			return
		}
		yield(filepath.Join(homeDir, ".dockercfg"))
	}
}
